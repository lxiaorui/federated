...
def construct_federated_setattr_call(federated_comp, name, value_comp):
  """Returns CBB for `setattr(name, value_comp)` on `federated_comp`.
...
def construct_named_tuple_setattr_lambda(named_tuple_signature, name,
  ...
  Returns:
    An instance of `computation_building_blocks.Lambda` representing setting
    attribute `name` to value `value_comp` in its argument of type
  ...
  elem_array = []
  for idx, (k,
            v) in enumerate(anonymous_tuple.to_elements(named_tuple_signature)):
    if k == name:
      if not type_utils.is_assignable_from(v, value_comp.type_signature):
        raise TypeError(
            'Setattr has attempted to set element {} of type {} with incompatible type {}'
            .format(k, v, value_comp.type_signature))
      elem_array.append((k, value_comp))
    else:
      elem_array.append(
          (k, computation_building_blocks.Selection(lambda_arg, index=idx)))
  return_tuple = computation_building_blocks.Tuple(elem_array)
  return computation_building_blocks.Lambda('lambda_arg', named_tuple_signature,
                                            return_tuple)
