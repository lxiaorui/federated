...
class ValueImplTest(parameterized.TestCase):
  ...
  def test_setattr_federated_named_tuple_type_with_unnamed_element(self):
    ...
    with context_stack_impl.context_stack.install(
        reference_executor.ReferenceExecutor()):
      self.assertEqual(
          foo([[5, 1.0, True], [0, 2.0, True],
               [-5, 3.0, False]]), [
                   anonymous_tuple.AnonymousTuple([('a', 5), (None, 1.0),
                                                   ('b', False)]),
                   anonymous_tuple.AnonymousTuple([('a', 0), (None, 2.0),
                                                   ('b', False)]),
                   anonymous_tuple.AnonymousTuple([('a', -5), (None, 3.0),
                                                   ('b', False)])
               ])
